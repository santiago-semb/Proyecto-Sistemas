        ...
        data.forEach(cat => {
            divCat.innerHTML += `
                <div class="font-bold m-2">
                    <button class="text-xs rounded p-1 text-white bg-red-500 font-bold" onclick="buscarOfertaPorCategoria(${cat.Id})">
                        ${cat.Nombre}
                    </button>
                </div>
            `
...
const buscarOfertaPorCategoria = async (categoria) => {
    const API_URL_cat = `http://localhost:${port}/api/Categoria/${categoria}`;
    let dataCat = await fetchApi2(API_URL_cat, "GET");
    let nombreCategoria = dataCat.Nombre;
    ...
    if(data[0] === undefined || data[0] === null)
    {
            ...
            <div class='text-center mx-auto mt-24'>
                <i class="fa-solid fa-face-sad-tear text-gray-800" style='font-size: 100px;'></i>
                <p class="text-2xl text-gray-900 my-4">No se encontraron ofertas de <b>${nombreCategoria}</b></p>
                <p clas="text-2xl"><button onclick="obtenerOfertas()" class="bg-gray-200 text-black border-2 border-gray-800 hover:bg-gray-300 px-2 py-1 rounded">Volver</button></p>
            </div>
        `
    }
